...
// The error domain that all errors generated by XCryptoRequest use
#define XCryptoRequestErrorDomain (@"XCryptoRequestErrorDomain")

@interface XCryptoRequest(){
...
@property(nonatomic,assign)XCryptoErrorType erType;//error type
...
-(void)start{
    ...
    @try {
        ...
        NSError *er = [NSError errorWithDomain:XCryptoRequestErrorDomain
...
-(void)succeededXcrypt{
    ...
    if(self.delegate &&
       [self.delegate respondsToSelector:@selector(succeededXCryptoRequest:)]){
    
        [self.delegate succeededXCryptoRequest:self];
    }
}
...
-(void)failedXcrypt{
    ...
    if(self.delegate &&
       [self.delegate respondsToSelector:@selector(failedXCryptoRequest:)]){
        
        [self.delegate failedXCryptoRequest:self];
    }
}

-(void)collectionErrorInfo{

    self.er=[[NSError alloc] initWithDomain:XCryptoRequestErrorDomain code:self.erType userInfo:nil];
}
